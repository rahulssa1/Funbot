...
if (!OPENROUTER_API_KEY) {
    console.error(red(bold('ERROR: Please set the OPENROUTER_API_KEY variable.')));
    process.exit(1);
}
const openai = new OpenAI({
    ...
    defaultHeaders: {
        "HTTP-Referer": "https://mrptech.local",
        "X-Title": "MRPTech Quiz Bot"
    }
});
...
const askAiForAnswer = async (question, options) => {
    ...
    console.log(yellow('\n--- 🤔 Asking DeepSeek V3 via OpenRouter ---'));
        ...
        try {
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a quiz-solving AI. Reply with ONLY a single non-negative integer (0-based index of the correct option)."
                    },
