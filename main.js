...
if (!OPENROUTER_API_KEY) {
    console.error(red(bold('ERROR: Please set the OPENROUTER_API_KEY environment variable.')));
    process.exit(1);
}
const openai = new OpenAI({
    apiKey: OPENROUTER_API_KEY,
    baseURL: "https://openrouter.ai/api/v1",
    // Optional but recommended: app attribution headers
    defaultHeaders: {
        "HTTP-Referer": "https://mrptech.local", // your app/site URL (any string is okay if local)
...
const askAiForAnswer = async (question, options) => {
    ...
    console.log(yellow('\n--- 🤔 Asking DeepSeek V3 via OpenRouter (Once) ---'));
