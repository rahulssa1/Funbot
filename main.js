const fs = require('fs'); const path = require('path'); const axios = require('axios'); const zlib = require('zlib'); const chalk = require('chalk');

const customChalk = new chalk.Instance({ level: 3 }); const green = customChalk.green; const yellow = customChalk.yellow; const red = customChalk.red; const cyan = customChalk.cyan.bold; const bold = customChalk.bold; const gray = customChalk.gray;

const DEEPSEEK_API_KEY = "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";

const POLL_INTERVAL_S = 95; const AI_MAX_RETRIES = 7; const AI_RETRY_DELAY_S = 5; const GET_TIMEOUT_S = 7000; const POST_TIMEOUT_S = 25000;

const NUM_WORDS = { zero: 0, one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9, };

const api = axios.create({ headers: { 'Accept': 'application/json, text/plain, /', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36', }, });

const printBanner = () => { const lines = [ "     Auto Quiz Bot     ", "      MRPTech     " ];
...
const fetchQuizState = async (lobbyId) => { try { const url = https://ft.games/api/quiz/${lobbyId}/; const response = await api.get(url, { timeout: GET_TIMEOUT_S, responseType: 'arraybuffer', headers: { 'Referer': https://ft.games/${lobbyId}/quiz/ } }); const encoding = response.headers['content-encoding']; const data = encoding === 'br' ? zlib.brotliDecompressSync(response.data) : response.data; const decoded = JSON.parse(data.toString('utf8')); return { lobbyId, state: decoded, error: null }; } catch (error) { return { lobbyId, state: null, error: error.message }; } };

const aiPrompt = (question, options) => { const nonAlnumChars = (question.match(/[^a-zA-Z0-9\s]/g) || []).length; const isEmojiQuestion = question.length > 0 && nonAlnumChars / question.length > 0.4; const questionText = isEmojiQuestion ? These emojis represent a concept...\n${question} : question; const formattedOptions = options.map((opt, i) => [${i}] ${opt}).join('\n'); return You are a quiz-solving AI... Answer ONLY as a single digit...\n\nQuestion: ${questionText}\n\nOptions:\n${formattedOptions}; };

const askAiForAnswer = async (question, options) => { const prompt = aiPrompt(question, options); console.log(yellow('\n--- 🤔 Asking DeepSeek API ---'));
...
const submitAnswer = async (submitOrder) => { const { lobbyId, quizId, answerIndex } = submitOrder; try { const url = https://ft.games/api/quiz/${lobbyId}/; const payload = { quiz_id: quizId, sel_idx: answerIndex }; const response = await api.post(url, payload, { timeout: POST_TIMEOUT_S, headers: { 'Referer': https://ft.games/${lobbyId}/quiz/ } }); const isOk = JSON.stringify(response.data).toLowerCase().includes('ok'); console.log(green(✔ Submitted to lobby. Success: ${isOk})); } catch (error) { console.error(red(✖ Submission failed for lobby: ${error.message})); } };

const main = async () => { printBanner();
...
process.on('SIGINT', () => { console.log(yellow('\n\n👋 Closing bot. Goodbye from MRPtech')); process.exit(0); });

main().catch(err => { console.error(red('\nAn unexpected error occurred:'), err); process.exit(1); });
